    ...
    public static class LogWriter
    {
        ...
        public static void Initialize()
        {
            try
            {
                ...
                _zoneThread = new Thread(zoneMain) { IsBackground = true, Priority = ThreadPriority.BelowNormal };
    ...
    class OCRReader
    {
        ...
        public static void InitializeOCR()
        {
            try
            {
                ...
                Plugin.LogParserMessage("Tesseract OCR engine initialized.");
            }
            catch (Exception ex)
            {
                Plugin.LogParserMessage("Error initializing Tesseract OCR engine: " + ex.Message);
            }
        }
        ...
        public static void CaptureZoneName()
        {
            ...
            if (hWnd == IntPtr.Zero)
            {
                Plugin.LogParserMessage("BNSR window not found.");
                return;
            }
            RECT rect;
            ...
            while (true)
            {
                try
                {
                    ...
                    using (Graphics graphicsBitmap = Graphics.FromImage(bitmap))
                    {
                        ...
                        {
                            Plugin.LogParserMessage("PrintWindow failed.");
                ...
                {
                    Plugin.LogParserMessage("Error capturing zone name: " + ex.Message);
                    Thread.Sleep(2000);
                }
            }
        }
